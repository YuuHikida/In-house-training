...
//        return "manager/report-detail";
    ...
    public String display(Model model) {
        ...
        String title = "ユーザー作成";
        model.addAttribute("title", title);
        List<Team> teamList = teamService.getAllTeam(); //←DBに問い合わせてチームのidとnameを取得(複数)
...
//--テスト範囲-------------------------------------------
    ...
    public String displayDeleteUser(@RequestParam("employeeCode") int employeeCode,
        ...
        settingService.deleteById(employeeCode);
        applyService.deleteAll(employeeCode);
...
//        redirectAttributes.addFlashAttribute("addCompleteMSG", "始業時間を『" + setting.getStartTime() + "』、終業時間を『" + setting.getEndTime() + "』に設定しました。");
    ...
    public String displayApplySearch(
        ...
        //報告一覧表示---------------------------------
        //自分が管理するチームIDを取得する
        List<Assignment> teamList = assignmentService.getAsManager(employeeCode);
        List<Apply> applyList =  new ArrayList<>();
        for (Assignment team : teamList){
            List<Apply> applys = applyService.getOtherMembers(team.getTeamId());
            for (Apply apply : applys){
                User user = userService.getUserByEmployeeCode(apply.getEmployeeCode());
                apply.setUser(user);
            }
            applyList.addAll(applys);
        }
    ...
    public String applyDetail(
        ...
        return "/manager/apply-detail";
    }

    @GetMapping("/applyApproval/{applyId}/{approval}")
    public String applyApproval(
        ...
        //決裁結果をDBに登録する
        applyService.updateApproval(applyId, approval);
        Apply apply = applyService.findById(applyId);
        apply.setUser(userService.getUserByEmployeeCode(apply.getEmployeeCode()));
        model.addAttribute("apply",apply);
        return "redirect:/manager/apply-search";
    }
