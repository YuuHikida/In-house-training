...
//    }
    ...
    public void deleteAll(int employeeCode){
        this.applyRepository.deleteAll(employeeCode);
    }
    ...
    public List<Apply> getOtherMembers(int teamId) {
        List<Apply> applys = applyMapper.getOtherMembers(teamId);
        // createdDate をフォーマットする
        applys.forEach(apply -> apply.setFormattedCreatedDate(formatDateTime(apply.getCreatedDate())));
        return applys;
    }
    ...
    public void updateApproval(int applyId, int approval){
        this.applyRepository.updateApproval(applyId, approval);
    }
