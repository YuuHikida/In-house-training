...
public class TaskService {
    ...
    //[追加機能]ログイン中のユーザーのタスクのみを追加
    public void addTask(TaskInputForm taskInputForm)
    {
        //taskInputList配列のnull要素を弾く
        List<UserTaskInput> taskList = taskInputForm.getTaskInputList().stream()
                .filter(x -> x.getTask_name() != null && !x.getTask_name().isEmpty())
                .toList();

        //DB登録用のtaskModelに値を入れ込む
        for(UserTaskInput x : taskList)
        {
            Task task = new Task();

            task.setEmployeeCode(getAuthenticationEmployeeCode());
            task.setTaskId(taskRepository.selectMaxTaskId()+1);
            task.setTaskName(x.getTask_name());
            task.setComment(x.getComment());
            task.setProgressRate(x.getProgressRate());
            task.setCreateAt(LocalDate.now());
            task.setUpdateAt(null);

            taskMapper.saveUserTask(task);
        }
